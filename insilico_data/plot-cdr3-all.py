...
for sample in AllSamples:
    ...
    trust_fq_stats = get_stats(parse_trust(sample, TRUST_FQ_PATH), prefix='trust4_fq_')
    print( sample )
    trust_stats = get_stats(parse_trust(sample, TRUST_PATH), prefix='trust4_bam_')
    print( "done trust" )
    #AllStats.append(merge_dicts(sample, mixcr_stats, catt_stats, trust_stats))
    AllStats.append(merge_dicts(sample, mixcr_stats, mixcr_badq_stats, catt_stats, trust_fq_stats, trust_stats))
...
def plot_false_positive_in_silico_left_only(paired=True, star_reference = 'hg38', ax = None):
    ...
    softwares = ['MiXCR_Default', 'MiXCR_0', 'CATT', 'TRUST4_FQ', 'TRUST4_BAM']
    software_offset = 2.5
    width           = 1.5 
