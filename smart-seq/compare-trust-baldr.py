...
parser = argparse.ArgumentParser(description="Compare TRUST4 and BALDR", formatter_class=argparse.ArgumentDefaultsHelpFormatter, usage='%(prog)s [options]', add_help=True)
...
for file in natural_sort(glob.glob(trust_dir+"*_report.tsv")):
    ...
    baldr_heavy_chains = 0
    baldr_light_chains = 0  
    # Open equivalent BALDR heavy chain output 
    igFileExt="_1.IG-mapped_Unmapped.igblast_tabular.quant.sorted"
    with open(baldr_dir+filename.split("_report.tsv")[0]+igFileExt+".IGH.filtered") as f:
        lines = f.readlines()
        seq = lines[0].split('\t')[61]
        trust4_seq = ''
            ...
            for name2, seq2 in read_fasta(f2):
                if name2.split(' ')[0] == '>'+assembly_id_heavy:
                    ...
                    if name2.split(' ')[3] == '*':
                        ...
                        if v_end >= cdr3_start:
                            v_length_ovr = int(v_length_ref - (v_end - cdr3_start + 1))
                        else:
                            v_length_ovr = v_length_ref
                        v_length = v_length_ovr - (v_length_ref - v_ref_end - 1)
                    ...
                    if name2.split(' ')[5] == '*':
                        ...
                        if j_start <= cdr3_end:
                            j_length_ovr = int(j_length_ref - (cdr3_end - j_start + 1))
                        else:
                            j_length_ovr = j_length_ref
                        j_length = j_length_ovr - j_ref_start
                    break
        ...
        if trust4_seq != '' and trust4_chain1 in seq:
            ...
            left_side_baldr = seq.split(trust4_chain1)[0]
            right_side_baldr = seq.split(trust4_chain1)[1]

            left_length = compare(rev(left_side_trust4),rev(left_side_baldr))

            right_length = compare(right_side_trust4,right_side_baldr)
            ...
            baldr_heavy_chains +=1                     
   
    # Open equivalent BALDR light chain output 

    with open(baldr_dir+filename.split("_report.tsv")[0]+igFileExt+".IGKL.filtered") as f:
        lines = f.readlines()
        seq = lines[0].split('\t')[58]
        trust4_seq = ''
        with open(trust_dir+filename.split("_report.tsv")[0]+"_annot.fa") as f2:
            for name2, seq2 in read_fasta(f2):
        ...
        if trust4_seq != '' and trust4_chain2 in seq:
            ...
            left_side_baldr = seq.split(trust4_chain2)[0]
            right_side_baldr = seq.split(trust4_chain2)[1]
            ...
            baldr_light_chains +=1

    if baldr_heavy_chains == 0:
        IGH_V.append("x")
        IGH_J.append("x")          
    if baldr_light_chains == 0:  
        IGKL_V.append("x")
        IGKL_J.append("x") 
